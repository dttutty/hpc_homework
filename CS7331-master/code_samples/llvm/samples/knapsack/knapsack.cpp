...
void looprun(int **itemsjo, int threads) {

  int mainLB;

  for (int K = 1; K <= number_items; K = K + threads) { 
    
#ifdef DEBUG
    printf("Starting next group of items ...\n");
#endif 
    /* preloop */
    for (int t = K, i = 1; t < K + threads; t++, i++) {
#ifdef DEBUG
      if (threads - i != 0)
	printf("Item = %d\tBlock : 1:%d\n", t, BLOCK * (threads - i));
#endif
      for (int xx = 1; xx <= BLOCK * (threads - i); xx++) {
	if(xx < weight[t])
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 3) {
    cout << "usage: " << endl;
    cout << "\t./knapsack input_file num_threads" << endl;
    exit(0);
  }
#ifdef DEBUG
  ...
  cout << "Running knapsack with " << threads << " threads." << endl;
  cout << "Items: " << number_items << "   Capacity: "<< vessel_cap << endl;
  ...
  cout<< "Results saved in file results.dat " <<endl; 
